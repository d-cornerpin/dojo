...
export const Memory = () => {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Top bar */}
      <div className="flex items-center justify-between px-6 py-4 border-b white/[0.06]">
        <div className="flex items-center gap-4">
          <h2 className="text-lg font-semibold text-white">Memory Browser</h2>
          <select
            value={agentId}
            onChange={(e) => setAgentId(e.target.value)}
            className="bg-white/[0.05] border white/[0.08] rounded-lg px-3 py-1.5 text-sm white/90 focus:outline-none focus:ring-1 focus:ring-blue-500"
          >
            {agents.map((a) => (
          ...
          <button
            ...
            className={`px-3 py-1.5 text-xs rounded-lg transition-colors ${
              rightPanel === 'search'
                ...
                : 'bg-white/[0.05] white/55 hover:white/90'
            }`}
          >
            Search
          </button>
      ...
      <div className="flex-1 flex min-h-0">
        {/* Left panel - DAG tree */}
        <div className="w-80 border-r white/[0.06] flex flex-col bg-white/[0.02]">
          <div className="px-4 py-2 border-b white/[0.06] flex items-center justify-between">
            <span className="text-xs font-medium white/40 uppercase tracking-wider">
              Summary DAG
            </span>
        ...
        <div className="flex-1 flex flex-col bg-transparent min-w-0">
          ...
          {rightPanel === 'briefing' && (
            <BriefingView agentId={agentId} />
          )}
          {rightPanel === 'none' && (
            <div className="flex items-center justify-center h-full white/30 text-sm">
      ...
      {/* Bottom bar */}
      <div className="flex items-center justify-between px-6 py-3 border-t white/[0.06] bg-white/[0.02]">
        <div className="flex items-center gap-2">
          <button
            onClick={() => setInjectOpen(!injectOpen)}
            className="px-3 py-1.5 text-xs rounded-lg bg-white/[0.05] white/70 hover:bg-white/[0.08] transition-colors"
          >
            Inject Memory
          </button>
          <button
            ...
            disabled={compacting}
            className="px-3 py-1.5 text-xs rounded-lg bg-white/[0.05] white/70 hover:bg-white/[0.08] transition-colors disabled:opacity-50"
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 text-xs rounded-lg transition-colors ${
              rightPanel === 'briefing'
                ? 'bg-blue-600/20 text-blue-400'
                : 'bg-white/[0.05] white/70 hover:bg-white/[0.08]'
            }`}
          >
            View Briefing
          </button>
