...
export const MicrosoftActivityLog = () => {
            ...
            <tbody>
              {entries.map(entry => (
                <tr key={entry.id} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
                  ...
                  <td className="py-1.5 pr-2 text-white/60">{entry.agentName ?? entry.agentId.slice(0, 8)}</td>
                  <td className="py-1.5 pr-2 text-white/70">{formatAction(entry.action)}</td>
                  <td className="py-1.5 pr-2">
                    ...
                    {!entry.success && (
                      <span className="ml-1 px-1.5 py-0.5 rounded text-[10px] bg-cp-coral/10 text-cp-coral border border-cp-coral/20">failed</span>
                    )}
                  </td>
