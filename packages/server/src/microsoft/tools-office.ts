...
export const officeToolDefinitions: ToolDefinition[] = [
  ...
  },
  {
    ...
    // PHASE-6 T0C: four of these five are an explicit NEGATIVE advertisement —
    // "There is NO sheet-level `formulas`, `columns`, `header_row`, or
    // `currency_format` field" — which is the opposite of a false promise and
    // must not be deleted to satisfy a scan. `formula` is a CELL-OBJECT key
    // described in prose inside `sheets[].rows`.
    advertisedNotDeclared: {
      formulas: 'named only to say it does NOT exist — an explicit negative advertisement, the opposite of a false promise',
      columns: 'named only to say it does NOT exist — an explicit negative advertisement',
      header_row: 'named only to say it does NOT exist — an explicit negative advertisement',
      currency_format: 'named only to say it does NOT exist at SHEET level — an explicit negative advertisement; it is a cell-object key',
      formula: 'a key of the cell object described in prose inside `sheets[].rows`, not a parameter of this tool',
    },
    nonEffects: {
      filename: 'a bare NAME, not a path: the handler sanitises it and writes into the agent uploads directory, which the fs_write effect above declares (the OneDrive branch writes no file at all). Resolved as a path it would name a file in the server working directory that this tool never touches — the false resolution RULING P5-R15 ADDENDUM 2 named',
    },
