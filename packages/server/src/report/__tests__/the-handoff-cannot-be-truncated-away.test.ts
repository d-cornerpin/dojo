...
//     evidence. Truncation from the end then cannot reach them, whatever the bundle does.
...
//     tokens. The budgets are chosen so a MAXIMAL bundle lands 6,874 tokens against a 12,000
//     cap, and the clauses below assert that arithmetic rather than trusting the comment.
//  3. HONESTY. A bound that drops evidence silently teaches the reader it saw everything, so
//     each bounded section reports `showing newest K of N`, and K is checked against the drop
...
//   M1  hand-off moved back to the tail of the gather result   → 3 F / 8 P  (clauses 1, 2, 3)
//   M2  the `logs` character budget removed                    → 4 F / 8 P  (clauses 3, 5, 8, 9)
//   M3  the "NOTHING IS FILED…" truth line deleted            → 2 F / 10 P (clauses 1, 3)
//   M4  the tool's own arguments left in the logs slice        → 1 F / 11 P (clause 10)
// M2's own words are the red restated by the suite: *"a maximal gather result is 35130 tokens
// against a 9000 target"* — 2.9× the engine's cap, from one unbounded section. The full
// messages are in `.superpowers/sdd/DOJO-REPORT-PLAN/ritual-red1-fix-report.md`.
// ════════════════════════════════════════════════════════════════════════════════════════
...
import { BUNDLE_SECTION_CHARS, COLLECTOR_CAPS, REPORT_RESULT_TARGET_TOKENS } from '../window.js';
...
describe('the evidence bundle is bounded by arithmetic, not by hope', () => {
  ...
  it('9 — the drop note is true of the drop that actually happened', async () => {
    ...
    const m = /logs: showing newest (\d+) of (\d+)/.exec(note!);
...
describe('a report about a report refers to it by id and never re-swallows its text', () => {
  ...
  it('10 — this tool\'s own arguments never come back as evidence, and the call still does', () => {
    seedOversize(20);
  ...
  it('11 — the discriminator names a tool that still exists, so a rename cannot make it vacuous', () => {
    expect(registryToolDefinitions().some(d => d.name === 'dojo_report')).toBe(true);
  });
});

describe('the draft phase obeys the same ordering law', () => {
  it('12 — `submit` is named before the attachment that can grow past the cap', async () => {
    seedOversize(20);
