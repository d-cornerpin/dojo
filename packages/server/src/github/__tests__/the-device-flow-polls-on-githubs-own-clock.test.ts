...
describe('the token reaches no log line, no broadcast frame, and no response body', () => {
  ...
  const SPELLINGS: ReadonlyArray<readonly [string, string, string[]]> = [
    ...
    // ── T5 FIX ROUND 1 (review F3a): two more the reviewer found OUTSIDE this table ──
    // Recorded as rows rather than left in prose, because the whole argument for this table is
    // that a blind spot nobody wrote down is a blind spot that stays. The ternary is the
    // realistic one — `type: ok ? 'github:connected' : 'github:connect_failed'` is an ordinary
    // refactor. Neither is a token risk: frame containment is held by the UNFILTERED sweep over
    // all ten endings, which does not care how the type was spelled.
    ['BLIND: a quoted key', "broadcast({ 'type': 'github:x' });", []],
    ['BLIND: a ternary', "broadcast({ type: ok ? 'github:a' : 'github:b' });", []],
    // ── SEEN, and worth pinning because the reviewer drove them and the scanner handled them ──
    ['a spread before the key', "broadcast({ ...base, type: 'github:x' });", ['github:x']],
    ['an `as` assertion after the object', "broadcast({ type: 'github:x' } as WsEvent);", ['github:x']],
    ['a deeply nested type is still not a frame type', "broadcast({ type: 'github:i', a: { b: { type: 'github:deep' } } });", ['github:i']],
  ];
