...
// ════════════════════════════════════════════════════════════════════════
// ARM 5 — ANSWER-ANYWAY: THE REFUSAL IS TIME-SHAPED, LIKE THE SIXTH
//
// The header above says the engine "knows at the instant". What it knows at the instant is
// that the ack is OWED and that these are the model's next words — never what the words SAY.
// The ritual measured the gap (v3.1.26 round 3, turn 5649, ask 695067c2): the model
// front-loaded its whole researched answer into the promoted bubble, ended honestly with
// `[no-reply]`, and this narrowing refused the ANSWER — four re-serves, a STOOD-DOWN ERROR
// and an owner ask left `blocked`, with his answer on screen throughout.
//
// So the rule keeps its instant and loses its blanket: mid-turn it is byte-identical (ARMS
// 1–4 above are the proof), and once the turn has ENDED the promoted bubble is judged by the
// one question every other `agent-text` bubble is judged by — does this finished turn's own
// answer key name it?
// ════════════════════════════════════════════════════════════════════════

describe('ARM 5 — once the turn has ended, the finished turn\'s own key decides', () => {
  it('RED→GREEN: a promoted bubble the finished turn CALLS its answer is evidence', () => {
    claimedAsk('m-1', 4);
    ackDelivery('d-ack', 'msg-ack', 'Fresh sources pulled just now — reMarkable $629, Boox $499…');
    finalizedTurn(4, 'msg-ack');
    expect(askAnswerEvidence(askIdForMessage('m-1'), 4)?.id).toBe('d-ack');
    settleAsksAtTurnFinalize({ agentId: AGENT, turnNumber: 4 });
    expect(workFor('m-1').state).toBe('done');
    expect(receiptOf('m-1')).toBe('d-ack');
  ...
  it('CONTROL — the same row MID-TURN is refused exactly as it was: the lying window cannot reopen', () => {
    claimedAsk('m-1', 4);
    ackDelivery('d-ack', 'msg-ack', 'Fresh sources pulled just now — reMarkable $629, Boox $499…');
    liveTurn(4, 0);   // the turn is still running: nobody can know which bubble was the answer
    expect(askAnswerEvidence(askIdForMessage('m-1'), 4)).toBeNull();
    expect(settleAsk(askIdForMessage('m-1'), { agentId: AGENT, turnNumber: 4, at: 'delivery' }).verdict)
      .toBe('unchanged');
    expect(workFor('m-1').state).toBe('claimed');
  ...
  it('CONTROL — a finished turn whose key names a DIFFERENT bubble still refuses the ack', () => {
    claimedAsk('m-1', 4);
    ackDelivery('d-ack', 'msg-ack', 'On it — I will look into that now.');
    bubbleDelivery('d-ans', 'msg-ans', 'Four tablets, all three dimensions.', { turn: 4, offsetSeconds: 8 });
    finalizedTurn(4, 'msg-ans');
    expect(askAnswerEvidence(askIdForMessage('m-1'), 4)?.id).toBe('d-ans');
  });

  it('CONTROL — no turn record at all is a refusal, never a naming', () => {
    claimedAsk('m-1', 4);
    ackDelivery('d-ack', 'msg-ack', 'Fresh sources pulled just now — reMarkable $629, Boox $499…');
    expect(askAnswerEvidence(askIdForMessage('m-1'), 4)).toBeNull();
  });

  // ⚠ THE NAMING CLAUSE IS THIS NARROWING'S OWN, NOT THE SIXTH ONE'S — and this clause is why
  // it is written here rather than leant on. The sixth narrowing refuses an unnamed bubble
  // only when the row reads `display_kind='agent-text'`; the promotion passes that kind
  // EXPLICITLY (UX-REPAIR T2, because `origin_intent` alone would classify the row `fallback`),
  // so a caller that ever stopped passing it would slip a stamped, unnamed ack past the sixth
  // narrowing entirely. Stating the rule where the rule lives — a stamped ack is evidence only
  // when the FINISHED TURN'S OWN KEY NAMES IT — means that slip cannot become a close.
  it('CONTROL — a stamped ack the sixth narrowing cannot reach is still refused when unnamed', () => {
    claimedAsk('m-1', 4);
    bubbleDelivery('d-ack', 'msg-ack', 'Fresh sources pulled just now — reMarkable $629, Boox $499…',
      { turn: 4, originIntent: START_ACK_INTENT, displayKind: 'fallback' });
    bubbleDelivery('d-ans', 'msg-ans', 'Four tablets, all three dimensions.', { turn: 4, offsetSeconds: 8 });
    finalizedTurn(4, 'msg-ans');
    expect(askAnswerEvidence(askIdForMessage('m-1'), 4)?.id).toBe('d-ans');

    // …and with NO other bubble to fall back on, it is refused outright rather than closing
    // the ask on an acknowledgment nobody called the answer.
    mockDb.current!.prepare(`DELETE FROM deliveries WHERE id = 'd-ans'`).run();
    finalizedTurn(4, null);
    expect(askAnswerEvidence(askIdForMessage('m-1'), 4)).toBeNull();
  });
});
