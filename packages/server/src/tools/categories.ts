...
function generateToolIndexInternal(
  ...
  if (compact && !isPrimaryClass) {
    ...
    lines.push('- **For tasks that span many sources or produce a long output** (read 5+ files then synthesize, build a long doc from a corpus, walk through a complex project): **your context is a FINITE window.** When it fills, older turns get summarized — file contents you read 30 minutes ago become a one-line summary that loses literal detail. An agent writing a flowchart from 100+ pages of summarized source **WILL fabricate plausible-sounding details that aren\'t there.** Defenses, in order: (1) `tracker_create_project` BEFORE you start, with batches as tasks. (2) Scaffold the deliverable first via `file_write` (section headers + placeholders). (3) Loop: read 3-5 sources, write findings via `file_append` (end-of-file) or `file_patch` (between markers), update `scratchpad_set`, mark the tracker task complete, move on. (4) Re-read the scaffold and scratchpad when lost — they survive compaction. (5) End with a verification pass. NOTE: the engine enforces this — after 6 loading calls without any structuring call (tracker_create_*, file_write/append/patch, scratchpad_set) the next loading call is refused.');
    lines.push('- **Close out tracker tasks the moment you finish them.** Don\'t end a turn with `in_progress` tasks you\'ve actually completed. For multi-step projects use `tracker_complete_step` (auto-advances). For standalone tasks use `tracker_update_status(complete)`. Blocked → mark blocked. Paused → mark paused. The engine catches dangling `in_progress` tasks at end-of-turn and nudges you; ignoring the nudge means the PM agent pokes you 30 minutes later. Both cost a turn the user is waiting on.');
  } else {
