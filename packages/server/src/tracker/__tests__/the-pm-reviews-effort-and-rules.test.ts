...
//      `advancing=false` fires the EXISTING rung-2 ("urgent") poke with the PM's reason
//      embedded, and ALSO moves the baseline so a stalled intervention re-reviews after
//      another threshold's worth of calls rather than spinning every sweep.
//
...
import { queueEffortReviewIfNeeded, runEffortReview } from '../pm-agent.js';
...
describe('§2 runEffortReview — advancing=true extends silently, advancing=false intervenes', () => {
  ...
  it('advancing=true: writes the observation, moves the baseline, and sends NO poke', async () => {
    ...
    handleMessageImpl = async (agentId) => {
      pmReplies(agentId, '{"advancing": true, "reason": "400-email sweep, labels are landing steadily"}');
    };
  ...
  it('advancing=false: fires the rung-2 ("urgent") poke with the PM reason embedded, and moves the baseline', async () => {
    seedOverThreshold('circ-1');
    handleMessageImpl = async (agentId) => {
      pmReplies(agentId, '{"advancing": false, "reason": "same three calls repeated with nothing new landing"}');
    };
    ...
    expect(pokeCount('circ-1')).toBe(1);
    ...
    `).get('circ-1') as { rung: number; poke_type: string };
    ...
    expect(envelope.payload).toContain('same three calls repeated with nothing new landing');
  });
  ...
  it('the assignee is never sent anything, and no steer enters its turn, on the advancing=true path', async () => {
    ...
    handleMessageImpl = async (agentId) => {
      pmReplies(agentId, '{"advancing": true, "reason": "still going, still landing real progress"}');
    };
