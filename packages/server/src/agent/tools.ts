...
export const toolDefinitions: ToolDefinition[] = [
  ...
  {
    name: 'imessage_send',
    description: 'Send an iMessage from YOUR OWN iMessage account (the DOJO bridge). **This is the ONLY way to deliver a message to anyone on iMessage — nothing you type in dashboard chat is auto-forwarded.** When the dashboard user is "out of the dojo" / away, this is also the only way to reach them at all; the dashboard is invisible to them in that state.\n\nVOICE: write like an actual text message. No markdown, no headers, no bullet lists, no code blocks. Short and conversational. The recipient is reading this on their phone.\n\nBAR FOR SENDING: only call this when there is a real thing the recipient needs or wants to receive — real outbound news, a real answer to a question they asked, something requiring their input or decision, or a direct reply inside an active iMessage thread. DO NOT call this for: acknowledgments ("Got it", "Noted", "On it", "All set"), self-narration ("Standing by", "Waiting on his reply", "That\'s the honest answer he deserved"), tool-completion summaries ("Done", "Done. Locked in.", "All three cleared", "All wrapped"), echoes of notifications you already saw, status reports nobody asked for, or restatements of info you already sent. When in doubt, do not send — silence is the right default. The dashboard chat is your working space for thinking, narration, and acknowledgments; the iMessage channel is reserved for deliberate delivery.\n\nCRITICAL recipient rule: when replying to an inbound iMessage, OMIT `recipient` — the tool defaults to the person who actually sent you the inbound, not the starred contact. Only pass `recipient` explicitly when proactively messaging someone (no inbound trigger), and the value MUST exactly match a safe-sender address. If you only know the person by name (e.g. user said "text Kevin"), call `imessage_list_contacts` first to look up the address, then pass it here. Passing an unknown address is refused. Supports text-only and text + file attachments (any local path, e.g. ~/.dojo/uploads/<agent-id>/photo.jpg) — files are sent via the imsg CLI; the message text rides with the first file.',
    input_schema: {
      type: 'object',
      properties: {
        recipient: {
